...
- Fix an empty dashboard for config files generated by pre-release builds. Those files numbered grid cells from 0 (`[0, 0]` top-left), so under today's 1-based coordinates every card fell outside the grid and was hidden. aitop now recognises such a file and shifts its positions by one; `[-1, -1]` still turns a provider off, and a file that declares itself 1-based is read as before.
